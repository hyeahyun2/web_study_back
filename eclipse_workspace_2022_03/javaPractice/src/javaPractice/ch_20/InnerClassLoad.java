...
class OutClass {
	 ...
	 * 자바16(2021년 3월)부터는 내부 클래스에서 정적 멤버를 선언할 수 있게 됨.
	 * */
	private int num = 10;
	...
	public OutClass() { // 외부 클래스의 디폴트 생성자
		// 내부 클래스 생성. 외부 클래스의 객체가 생성된 후에 내부 클래스의 객체 생성 가능
		inClass = new InClass(); 
	}
	class InClass { // 내부 클래스
		int inNum = 100;
		static int sInNum = 200; // 자바16 이전 버전에서는 에러 남
		
		void inTest() {
			System.out.println("OutClass num = " + num + "(외부 클래스의 인스턴스 변수)");
			System.out.println("OutClass sNum = " + sNum + "(외부 클래스의 스태틱 변수)");
			System.out.println("OutClass inNum = " + inNum + "(내부 클래스의 인스턴스 변수)");
			System.out.println("OutClass sInNum = " + sInNum + "(외부 클래스의 스태틱 변수)");
		}
		static void sTest() {
