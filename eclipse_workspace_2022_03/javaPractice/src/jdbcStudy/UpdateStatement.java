...
class DBUpdate extends DBSelect {
	protected User modifyUser() {
		...
		String validateTemp;
		do {
			System.out.println("정보를 수정 할 회원의 아이디를 입력하세요: ");
			user.userID = scanner.nextLine();
			System.out.print("회원의 이름을 입력하세요: ");
			user.name = scanner.nextLine();
			System.out.print("회원의 나이을 입력하세요: ");
			user.age = Integer.parseInt(scanner.nextLine());
			System.out.print("회원의 직업을 입력하세요: ");
			user.job = scanner.nextLine();
			
			System.out.println("수정 된 회원 : " + user.userID + " / "
	...
	public void updateUser() throws SQLException {
		...
		int cnt = preparedStatement.executeUpdate(); // 영향 받은 데이터 갯수 반환
