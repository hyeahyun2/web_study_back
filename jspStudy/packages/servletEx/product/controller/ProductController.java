...
public class ProductController extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(action == null) {
			// action 파라미터가 null인 경우, 즉 서블릿이 action 파라미터가 없이 호출된 경우 컨트롤러에 ㅁction 파라미터를 넣어 포워딩
			...
			// 이 때 request는 반드시 전달해야 파라미터 등을 읽어올 수 있으며
			// response의 경우 굳이 필요없으니 혹시 필요한 경우를 대비해 함께 전달
			switch(action) {
			case "list":
